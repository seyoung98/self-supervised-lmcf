...
class KITTIDC(BaseDataset):
    ...
    def __getitem__(self, idx):
        rgb, depth, gt, rgb_near, K, K_cam = self._load_data(idx)
        ...
        if self.augment and self.mode == 'train':
            ...
            if self.args.top_crop > 0:
                ...
                gt = TF.crop(gt, self.args.top_crop, 0,
                             height - self.args.top_crop, width)
                rgb_near = TF.crop(rgb_near, self.args.top_crop, 0,
                              height - self.args.top_crop, width)
                K[3] = K[3] - self.args.top_crop

            width, height = rgb.size

            # _scale = np.random.uniform(1.3, 1.8) # resize scale > 1.0, no info loss
            _scale = np.random.uniform(1.0, 1.5)
            # _scale = np.random.uniform(1.0, 1.3)
            scale = int(height * _scale)
            # scale =  int(self.height * 0.95) if scale >= self.height else scale
            ...
            rgb_near = TF.rotate(rgb_near, angle=degree, interpolation=InterpolationMode.BICUBIC)
            ...
            rgb_near = TF.adjust_brightness(rgb_near, brightness)
            rgb_near = TF.adjust_contrast(rgb_near, contrast)
            rgb_near = TF.adjust_saturation(rgb_near, saturation)
            ...
            rgb_near = TF.resize(rgb_near, scale, interpolation=InterpolationMode.BICUBIC)
            ...
            #     a=1
            # crop -> scale 된 rgb 이미지의 height가 패치 height보다 큰 경우
            ...
            if self.args.use_pose:
                success, r_vec, t_vec = get_pose_pnp(np.array(rgb), np.array(rgb_near), np.array(depth), K_cam[:, :3])
            ...
            rgb_near = TF.to_tensor(rgb_near)
            rgb_near = TF.normalize(rgb_near, (0.485, 0.456, 0.406),
                               (0.229, 0.224, 0.225), inplace=True)
        ...
        # -------------------- valid ------------------------------
            ...
            rgb_near = TF.crop(rgb_near, h_start, w_start, self.height, self.width)
        ...
        # ---------------------------------------------------------
            ...
            output = {'rgb': rgb, 'dep': depth, 'gt': gt, 'rgb_near': rgb_near,
                      'K': torch.Tensor(K)}

            return output

        output = {'rgb': rgb, 'dep': depth, 'gt': gt, 'rgb_near': rgb_near,
                      'K': torch.Tensor(K), 'r_mat': r_mat, 't_vec': t_vec}
    ...
    def _load_data(self, idx):
        ...
        path_rgb_near = os.path.join(self.args.dir_data,
                                     self.sample_list[idx]['rgb_near'])
        ...
        rgb_near = Image.open(path_rgb_near)
        ...
        return rgb, depth, gt, rgb_near, K, K_cam
