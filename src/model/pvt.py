...
class LiteMono(nn.Module):
    ...
    Lite-Mono
    """
    def __init__(self, in_chans=128, model='lite-mono', height=120, width=608,
        ...
        super().__init__()
        # Load the CF ResNet 34
        net = get_resnet34(pretrained=True)
        # setattr(self, "embed_layer1", net.conv1)
        # setattr(self, "embed_layer1", net.bn1)
        # setattr(self, "embed_layer1", net.relue)
        ...
        if model == 'lite-mono':
            ...
            elif height == 120 and width == 608:
                self.dilation = [[1, 2, 3], [1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]]

            elif height == 375 and width == 1242:
                self.dilation = [[1, 3, 7], [1, 3, 7], [1, 3, 7], [1, 3, 7, 1, 3, 7, 3, 6, 12]]

            elif height == 352 and width == 1216:
                self.dilation = [[1, 2, 7], [1, 2, 7], [1, 2, 7, 1, 2, 7, 2, 4, 14]]
            ...
            self.num_ch_enc = np.array([128, 128, 224])
            self.depth = [4, 4, 10]
            self.dims = [128, 128, 224]
            if height == 192 and width == 640:
                self.dilation = [[1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]]
            elif height == 320 and width == 1024:
                self.dilation = [[1, 2, 3], [1, 2, 3], [1, 2, 3, 1, 2, 3, 2, 4, 6]]
        ...
        self.downsample_layers = nn.ModuleList()  # stem and 4 intermediate downsampling conv layers
        stem1 = nn.Sequential(
        ...
        self.stem2 = nn.Sequential(
            Conv(self.dims[1]+64, self.dims[0], kSize=3, stride=2, padding=1, bn_act=False),
        )
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                Conv(self.dims[i]*2+64, self.dims[i+1], kSize=3, stride=2, padding=1, bn_act=False),
            )
    ...
    def forward_features(self, x):
        ...
        # fe2
        fe2 = getattr(self, 'embed_layer1')(x)
        features.append(fe2)

        # fe3
        fe3 = getattr(self, 'embed_layer2')(fe2)
        features.append(fe3)

        # input downsample
        # x_down = [fe3] # avgpooled input for every stage
        # for i in range(4):
        #     x_down.append(self.input_downsample[i](fe2))
        x_down = [self.input_downsample[i](fe2) for i in range(4)]

        # stage 1
        # x = self.downsample_layers[0](x)
        tmp_x = []

        x = self.stem2(torch.cat((fe3, x_down[0]), dim=1))
        tmp_x.append(x)
